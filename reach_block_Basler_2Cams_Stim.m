function reach_block_Basler_2Cams_Stim()
...
% Assign pseudorandomly trials with stimulation
stimTrials = randperm(number_of_trials,number_of_trials/2);
...
%         set(fh.tick,'String',['T  ',num2str(m),'m ',num2str(s),'s']);
    ...
    % A square pulse to trigger stimulation
    if ismember(trial,stimTrials)
        disp(['STIMULATED TRIAL ',num2str(trial)]);
        TDTsig3_n(1);
        pause(5);
    end
    
    %START OF A TRIAL/DOOR OPEN
    TDTsig_n(2);
    TDTsig2_n(1);  
    trial_time = tic;
...
save([path_f '\stimTrials_' video_name], 'stimTrials');
