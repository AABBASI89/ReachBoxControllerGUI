...
statusUpdate(['------------------------------'])
statusUpdate(['Starting new set of trials...'])
imaqmem(4e9)
...
number_of_trials = str2num(get(fh.trials ,'String'));
Iti = str2num(get(fh.Iti,'String'));
door_open_time = str2num(get(fh.doorWait,'String'));
...
path_f='C:\Users\TDT3\Desktop\reach_trials_2.0';
...
while(~stopState) && (trial < number_of_trials+1)
    ...
    stop(vid_lat);
    set(vid_lat,'DiskLogger',reach_vid);
    start(vid_lat);  
    ...
    if time_out == 0;
        ...
        while (pellet == 0);
            ...
            if ~exist('nopellet')
                nopellet = zeros(10,1);
                for n=1:length(nopellet)
                        nopellet(n) = readVoltage(a,1);
                end
                nopellet = mean(nopellet);
            end
            ...
            if readVoltage(a,1) < (nopellet-(0.2*nopellet))
                pellet = 1;
    ...
    back = tic;
%    disp('start');
    while (readVoltage(a,0)>thresh_back) && toc(back)<60
...
%         
%         TDTsig2_n(str2num(temp_trial(temp_n))+1);
%         pause(0.1)
%         
%     end
%     
...
while (readVoltage(a,1) < (nopellet-(0.1*nopellet)) && time_out == 0)
    ...
    if (readVoltage(a,1) < (nopellet-(0.1*nopellet)) && time_out == 0)
        time_out = 1;
        outcomes = [outcomes 3];
        pelletdrops= [pelletdrops NaN];
...
%             a.digitalWrite(8,1)
%             pause(0.05);
%             a.digitalWrite(8,0);  % real dispense
%         else
